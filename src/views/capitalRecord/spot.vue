...
export default {
  ...
  data() {
    return {
      ...
      requestUrl: "capitalRecordSpot",
      searchUrl: "capitalRecordSpotSearch",
      dynamicTable: [
        ...
        { name: "oldValue", value: true, label: "原有值" },
        { name: "amount", value: true, label: "变动值" },
        { name: "newValue", value: true, label: "现有值" },
        { name: "oldDeposit", value: true, label: "保证金原有值" },
        { name: "deposit", value: true, label: "保证金变动值" },
        { name: "newDeposit", value: true, label: "保证经现有值" },
    ...
    async setTableData(data) {
      ...
      data.forEach(function(element) {
        list.push({
          ...
          action: orderActions[element.Action],
          createTime: element.CreateTime.replace("T", " ").substring(0, 19),
          id: element.Id,
          newValue: this.numFilter(element.NewValue),
          oldValue: this.numFilter(element.OldValue),
          amount: this.numFilter(element.Amount),
          newDeposit: this.numFilter(element.NewDeposit),
          oldDeposit: this.numFilter(element.OldDeposit),
          deposit: this.numFilter(element.Deposit),
          detail: element
        });
      }, this);
      this.list = list;
